...
function getOne(id){
    return new Promise (function (resolve, reject) {
        PatientProfiles.findById(id).populate([{path: 'treatments', populate: {path: 'rehabilitationPlan', populate: [{path: 'exerciseOrders'}, {path: 'assessmentTests'}]}}, {path: 'country'}, {path: 'province'}, {path: 'gender'}]).exec(function (error, document) {
            if (error){
                reject(error);
            }else{
                resolve(document);
